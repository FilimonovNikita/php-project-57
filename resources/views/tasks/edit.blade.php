        ...
        <div class="grid max-w-screen-xl px-4 pt-20 pb-8 mx-auto lg:gap-8 xl:gap-0 lg:py-16 lg:grid-cols-12 lg:pt-28">
            <div class="grid col-span-full">
                ...
                <form action="{{ route('tasks.update', $task->id) }}" method="post">
                    @csrf
                    @method('PUT') <!-- Используйте PATCH метод для обновления записи -->
                    <div class="flex flex-col">
                        <label for="name" class="text-black dark:text-white">Name</label>
                        <input type="text" id="name" name="name" class="mt-2" value="{{ old('name', $task->name) }}">
                        @if ($errors->first('name'))
                            <div class="text-red-500">{{ $errors->first('name') }}</div>
                        @endif
                        <label for="description" class="text-black dark:text-white">Description</label>
                        <textarea id="description" name="description" class="mt-2">{{ old('description', $task->description) }}</textarea>
